...
public class Main {
    ...
    public static int stringToArray(String s) {
        ...
        if (strArr.length != 4) throw new ArrayIndexOutOfBoundsException("Кол-во строк в матрице не равно 4x4");
        String[][] matrixArr = {
                strArr[0].split(" "),
                strArr[1].split(" "),
                strArr[2].split(" "),
        ...
        for (int i = 0; i < matrixArr.length; i++) {
            for (int j = 0; j < matrixArr[i].length; j++) {
                try {
                    ...
                    throw new ArrayStoreException("В матрице,в строке "+i+" столбце "+j+" не число(символ/слово)");
                }
            }
        }for (int i = 0; i < matrixArr.length; i++) {
